...
## Phase 4.6 (expanded) — Environmental Hazards

The legacy checklist above has this as a single unexpanded line. Detail:

| Hazard | Behavior | Files | Notes |
|---|---|---|---|
| Sawblade (stationary) | Fixed position, spins in place, `x,y,w,h` rect, constant damage on overlap + knockback along the blade's tangent (not straight back — makes it read as a spinning object, not a wall) | `area.js`, `game.js` | Simplest to build; good first hazard to prototype the shared collision path other hazards reuse. |
| Timed crystal spikes | Telegraphed (crack/glow buildup ~20-30 frames) then erupt for a short window, retract, repeat on a fixed cycle | `area.js`, `game.js` | Reuses the same windup-color-ramp pattern already used for enemy telegraphs (`enemy.js` windup rendering) for consistency — players already know "color ramps to bright = don't stand here." |
| Moving pendulum blade | Swings on an arc between two angle bounds, `pivotX, pivotY, length, minAngle, maxAngle, speed` | `area.js`, `game.js` | Needs its own collision shape (line-segment-to-rect or a moving small rect sampled along the arc) since it's not axis-aligned like the others. |
| Damage aura floor (Static Field only) | Already partially speced (`expansion.md` 3.9 — touching the floor zaps you upward) — implement as a hazard type rather than a one-off special case, so future regions can reuse "floor is a hazard" as a configuration, not custom code. |

**Shared hazard object shape** (so `game.js`'s update/draw loop only needs one code path):
```js
{ type: 'sawblade'|'spike'|'pendulum'|'auraFloor', x, y, w, h, damage: 1, knockback: {x,y}, timer: 0, ...type-specific fields }
```
All hazards: no hitstun/flinch on the *player* beyond the standard hurt i-frames (`INVINCIBLE_FRAMES` already in `player.js`) — hazards should feel like environmental punishment, not combat, so they shouldn't interrupt combos the way an enemy hit does.

**Placement rule:** hazards belong inside the "big room" pattern (`expansion.md` 3.15) as a difficulty-curve tool — e.g. an easy lower path and a hazard-laden upper shortcut in the same room — not as their own dedicated rooms.

---

## New: Achievement Tracking

Not previously planned anywhere. Proposed scope:

| # | Task | Files | Description |
|---|------|-------|-------------|
| A.1 | **Achievement data model** | `game.js` | A flat list of achievement IDs with `{id, name, desc, hidden (bool), unlocked (bool), unlockedAt (timestamp)}`. Store in the existing save system (`stillpoint_save_v1` in `localStorage`) as a new `achievements` map — don't create a second storage key. |
| A.2 | **Trigger points** | `game.js`, `boss.js`, `enemy.js` | Hook achievement checks into existing event points that already fire once (ability pickups, Stillpoint activations, boss/miniboss defeats, area-discovered, full-lore-collected, no-hit boss clear) rather than polling every frame — reuse the same call sites `notifyHit`/`saveGame`/`switchArea` already use. |
| A.3 | **Toast notification** | `game.js` | A small canvas-drawn toast (reuse the existing ability-pickup notification system in `ability.js`'s `addAbilityNotification` pattern) — same visual language, not a new UI system. |
| A.4 | **Achievement list UI** | pause menu / new Journal (see below) | A tab inside the Journal/Inventory screen (not a separate menu) listing all achievements, with hidden ones shown as `???` until unlocked. |
| A.5 | **Starter achievement set** | design | Example seed list: first Stillpoint activation, first ability found, each miniboss defeated, King defeated, 100% lore, no-damage miniboss clear, defeat the King without using Stillpoint, find every breakable-wall secret in a region. Expand once Phase 3/4 content (expansion.md) is actually built — achievements should be added alongside new content, not retrofitted after. |

---

## New: Inventory / Journal Screen (Bestiary + Achievements + Upgrades)

Addresses: "an inventory where you look at achievements, enemy types (like Hollow Knight), and current upgrades."

| # | Task | Files | Description |
|---|------|-------|-------------|
| J.1 | **Journal screen shell** | `game.js`, `input.js` | New pause-menu-adjacent full-screen overlay (same drawing pattern as the map overlay in `map.js` — reuse `drawMap()`'s structure: fade background, centered panel, ESC/J to close). Bound to a new key (`J` suggested, doesn't conflict with current bindings) and also reachable from the pause menu. |
| J.2 | **Bestiary tab** | `game.js`, `enemy.js`, `boss.js` | Grid of enemy silhouettes (locked = solid black shape, no name; defeated-at-least-once = full color + name + HP + the one-line lore epitaph from `lore.md` Plan C if that direction is taken). Track `defeatedEnemyTypes: Set` in save data, incremented at the same `takeDamage`→`dead` transition point already in `enemy.js`/`boss.js`. |
| J.3 | **Achievements tab** | `game.js` | Pulls from the Achievement data model above (A.1). |
| J.4 | **Upgrades tab** | `game.js`, `player.js` | Shows every ability/upgrade with unlocked/locked state and a one-line description — this doubles as the fix for the current ability bar's `???`-only feedback (per the existing pause-menu ability display), giving a persistent place to see full kit state instead of only transient HUD icons. |
| J.5 | **Tab navigation** | `game.js`, `input.js` | Left/Right (or Q/E) to switch tabs, same input pattern as the pause menu's existing arrow-key navigation — no new input paradigm introduced. |

---

## New: Charged Attack as a Discovered Ability (not a starting ability)

Currently the charged/heavy attack (`player.js` — hold Z/J past `CHARGE_TAP`) is available from the very first frame of the game, with no gate. Proposed change:

| # | Task | Files | Description |
|---|------|-------|-------------|
| C.1 | **Gate the charge input** | `player.js` | Add `abilityState.hasHeavyAttack` (mirrors the existing `hasPhaseDash`/`hasShardShot` pattern in `ability.js`). Until unlocked, holding Z/J past `CHARGE_TAP` does nothing extra — only the normal tap-attack fires, no charge VFX/SFX plays, so an un-upgraded player doesn't feel like the button is broken, just that nothing special happens yet. |
| C.2 | **Heavy-attack-only walls as the delivery mechanism** | `area.js` | Directly reuses the "heavy-attack-only walls" hazard/puzzle type already added to `expansion.md` 3.16 — place the first one early (e.g. in a Fracture-adjacent room, visible but unbreakable with a normal hit, with a subtle "this doesn't crack like the others" visual cue) blocking a short side-path to the ability pickup itself. This makes the ability's discovery mechanically demonstrate its own power (you find out heavy attacks exist by noticing a wall your basic kit can't touch), same pattern Metroidvanias use for most movement upgrades. |
| C.3 | **Migration note** | `player.js`, `game.js` | The tutorial (`ROADMAP.md` 0.2) currently doesn't teach charge-attack at all — leave that as-is; charge attack should be found post-tutorial, not taught in it, consistent with it no longer being a day-one ability. |

**Depends on:** the damage-rebalance prompt (see chat) should land first, since it changes what "the heavy attack matters" actually means numerically — no point gating an ability behind a wall if its combat payoff is unclear once you get it.