...
| Anim-authored coverage | For each `ENEMY_REGISTRY`/`MINIBOSS_CLASSES` key, check whether **any** `ANIM_DEFS` key matches that entity's convention (`enemy_<id>_<state>` or `boss_<state>`/`boss_<telegraph.type>`, per `Plans/animation_editor_plan.md`'s bridge conventions) | A simple `Object.keys(ANIM_DEFS).some(k => k.startsWith('enemy_' + id + '_'))` per entity — cheap, no new data needed, `ANIM_DEFS` is already loaded. |
