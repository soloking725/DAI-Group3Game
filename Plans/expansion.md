...
**Status update, 2026-07-21 — 8 more built as `ComposedEnemy` defs
(`game/enemy.js`, registered in `ENEMY_REGISTRY`, spawnable/testable now):**
**#18 Void Juggernaut**, **#14 Ruin Stalker**, **#9 Fractured Knight**,
**#10 Shard Spitter**, **#12 Kinetic Striker**, **#11 Timeworn Husk**
(introduced a new generic `regen` trait for it — heal-over-time unless hit
recently, now authorable on any composed enemy via `enemy_designer.html`'s
"Regen" section), **#15 Pulse Warden**, and **#8 Stillpoint Revenant** (the
first enemy anywhere to actually use the `stillpoint/cancel` counter — it
existed in code since the original combat overhaul but had never been
attached to a real enemy). Some are simplified vs. their description here
(Ruin Stalker doesn't literally wall-cling — no such movement type exists
yet — it leans on Tiger Knee instead; Pulse Warden's rings are a flat
radius check, not a rising ring you jump over). None are placed in any
...
28. **Runner** - Dodges really well (expand this)
...
## PHASE 4: MINIBOSSES (12, rewritten 2026-07-22 from the user's own redone spec)
...
**Reward column corrected 2026-07-22**: the previous version of this table wrongly listed Mirror King as granting a "Graviton Surge upgrade" and Electromagnetic Golem as granting a "Shard Shot upgrade." Neither is true — per `regions.md`'s own ability-grant table, **Phase Dash comes from Mirror Veil Sanctum** (Mirror King's own fight — this was never a change, just previously mis-stated here), **Graviton Surge comes from Graviton Core** (the region, via room levers, not tied to a specific miniboss kill), and **Electromagnetic Golem grants nothing**. Fixed below; see `lore.md`'s "Minibosses & their regions" for the tragedy/moral-axis write-up behind each fight, cross-referenced here rather than duplicated.

| # | Name | Moral axis | Style of Attack | Phase 1 | Phase 2 | Phase 3 | Strategy | Reward |
|---|------|------------|------------------|---------|---------|---------|----------|--------|
| 4.1 | **The Mirror King** (Mirror Veil) | Evil-by-choice | Duplicates himself, the player, and projectiles | Lower mirror-copies of himself and copies of the player swarm the arena alongside duplicated projectiles | Once the real King is found and dropped, his copies stop fighting the player and turn on each other for what's left of his authority — clear the survivors | — | See through the swarm to find and hit the real one; not immune to knockback, so hard punishes work | **Phase Dash** (core ability grant — corrected, was never actually a Graviton Surge upgrade) |
| 4.2 | **Gravity Collapse Core** (Event Horizon) | No moral agent (accident/process) | Massive flying construct; changes room gravity to any of the 4 directions, generates hazards and moving-platform constructs | Debris projectiles, gravity shifts under the player | Same kit, denser hazard layering as HP drops | — | Immune to knockback and to being juggled — dance the shifting-gravity platforms and use Graviton Surge to escape bad pulls | +1 Fracture pip (max 4) |
| 4.3 | **Temporal Warden** (Chrono-Space Rift) | Sympathetic | Precognition-driven time mage; deliberately holds back his true strength out of guilt | Rewinds his own health on a visible countdown; you must deal enough damage during the brief flash window to interrupt it | — | — | Be aggressive specifically during the countdown flash; this is the one enemy with narratively-justified partial Stillpoint resistance (`expansion.md` §2 already flags this — don't repeat the treatment elsewhere) | Stillpoint upgrade (life steal +1 per hit); becomes a real ally once his love is freed from The Undertow (Void Expanse) |
| 4.4 | **The Fractured Sovereign's Guard** (Graviton Core) | Sympathetic (the one direct Sovereign-thread exception) | Strong, player-scale melee fighter | Shield bash and combos; can parry/block with the shield until it breaks | Drops rubble by hitting the ceiling; huge knockback on hits, somewhat fast, resistant to the player's own knockback | — | Parry the bash, punish while the shield's down or broken | +1 Max Health |
| 4.5 | **Electromagnetic Golem**, directed by an unnamed scientist (The Polar Shift) | Scientist: evil-by-choice | Strong hits with heavy knockback; the scientist gives walls, floor, or the player a magnetic charge (attract or repel), with counterplay platforms that flip the player's own charge | Repulsion fields push the player around the room | Charge-reversal slams pull the player in | — | Weak to projectiles and charged attacks; use the counterplay platforms to flip your own charge and create openings | None |
| 4.6 | **Quantum Pursuer** (Echoing Abyss) | Evil-by-choice | Releases a 0.5s-delayed shadow of the player's own soul that forces constant movement, while charging soul-powered ranged spells from range | Weak to knockback | Soul-charged and resistant to knockback; covers the field with long-range spells | — | Never stop moving to stay ahead of your own delayed shadow; punish hard in phase 1 before the resistance kicks in | +1 Dash Chain (max 4) |
| 4.7 | **Crag Warden** (Crag of the Colossus) *(built — `enemy.js`'s `ColossusCore` class, display name only)* | Evil-by-choice | Player-scale melee fighter; fused himself with the crag's own crystal-heart tech for permanent "super armor" | Telegraphed charge attack, only damaged by Charged (heavy) hits — a normal hit bounces off | Combos layered on the same charge pattern as HP drops | — | Only Charged Attacks work; immune to knockback; a landed hit can drive the player into the ground hard enough to need a mash-to-escape prompt | +1 Max Health |
| 4.8 | **The Assembler** (Paradox Engine) | Open — not yet placed | Extremely fast warp-speed attacks and portal openings; the creator herself, not an abandoned automaton | Standard telegraphed pattern (slam, shockwave, tracking beam) | Portal-assisted flanking attacks, punishing anything but patient cooldown-timed openings | — | Wait out her warp-speed windows before punishing | **Warp Key** (real postgame utility — see the Collapse-ending wasteland content) |
| 4.9 | **Warden & Hollow** (Warp Gate Nexus) *(mirror duo)* | Sympathetic (dutiful, not cruel) | Reactive counter-pair, not fake prediction | Warden: guaranteed parry vs. melee only, no other offense. Hollow: guaranteed dodge/teleport vs. ranged & ability hits, vulnerable to melee | — | — | Melee the ranged-dodger (Hollow), Shard Shot/Phase Dash the melee-parrier (Warden) — forces toolkit-switching | +1 Dash Chain, or a cosmetic reward |
| 4.10 | **The Conduit** (Static Field) | Open — not yet placed | Raw electricity; the creator herself, still working | Electrical arc attacks | Weaponizes the electroweak interaction directly — a slow, weak damage-over-time decay effect on the player | — | Open — not yet fully speced, flagged for expansion in `lore.md` | None (guards this region's Fracture Pip instead) |
| 4.11 | **The Stationmaster** *(proposed name, not locked)* (Timeline Crossroads) | Evil-by-choice | Trains, locomotives, and brainwashed prisoners | Prisoners (killable by his own other attacks, not just the player's) swarm while trains/locomotives fly through the arena | A flying, knockback-resistant phase — he takes a lot of knockback but keeps flying regardless | — | Clear or avoid prisoner waves, time hits around train sweeps | Immediate, real (if limited) **Void Tether** on defeat — no longer gated behind a second fight, see `story.md` §4 |
| 4.12 | **The Undertow** *(proposed name, not locked)* (The Void Expanse) | Open — not yet placed | Void attacks and projectiles; teleports/fast-travels through the void at will | Void projectile barrage | Teleport-flanking attacks, harder to read positioning | — | Track the teleport tells; this is the fight that frees the Temporal Warden's love | Completes the Void Tether (full range, no charge limit) if already held; opens a one-time lore-pip reassignment |

**Design note on 4.9 (Warden & Hollow):** this is the buildable version of a "this enemy reads me" boss (a Psycho Mantis-style idea) — each half is a simple, readable rule (not real prediction), so neither reads as unfair alone, but the pairing genuinely forces you to alternate tools. Cheap to build (two reaction checks), no new engine work needed.

**Non-individual encounters, not in the numbered list above:**
- **The Sovereign's Army** (Sovereign's Army Reserve) — not a named miniboss, a horde gauntlet: the same guard cloned over and over, frozen in time many loops ago, no will left but to fight. Mixed ranged/melee, cleared in sequence to reach the Limit Break.
- **The Child** (The Antechamber) — **status: conflicts with Abandoned Shell, not yet reconciled** (see `lore.md`'s own flag on this). If built: fights with tasers, flamethrowers, bombs, and guns scavenged from guards; 3 phases (alone → calls other former prisoners and heals them → rage boost).

**Final boss — The Sovereign** (matches `boss.js`'s existing 3-phase build, detailed in `lore.md`): player-scale moveset missing Graviton Surge (never collected it in her own time as the player), corrupted Void Tether pulls her to the player. Phase 1: heavy precognition, reads and counters the player's next move. Phase 2 (≤60% HP): full kit except Stillpoint. Phase 3 (≤30% HP): Stillpoint plus full immunity to the player's own.

**Scope note (2026-07-22):** each miniboss above now gets **three** fixed moveset instances — base game (this table), Sovereign postgame (`story.md` §9, deliberately different), and a Loop-ending/child-NG+ variant (same vibe, reflavored — see `lore.md`'s scope note). Never randomized or per-run.
