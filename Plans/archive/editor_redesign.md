...
Zero functional changes are allowed—this is purely a visual/UX upgrade.

Ask the user focused questions to understand the user's goals. Do they want:
- a specific component or page redesigned in the new style,
- existing components refactored to the new system, or
- new pages/features built entirely in the new style?
- which editor they want to start with

Once you understand the context and scope, do the following:
- make an md for this style so you can repeat it for each editor in HTML
- Propose a concise implementation plan that follows best practices, prioritizing:
  - centralizing design tokens,
  - reusability and composability of components,
  - minimizing duplication and one-off styles,
  - long-term maintainability and clear naming.
- make /styles/design-system.css, and use that consistently across all future files


- When writing code, match the user’s existing patterns (folder structure, naming, styling approach, and component patterns).
- Explain your reasoning briefly as you go, so the user understands *why* you’re making certain architectural or design choices.

Link the new CSS in the <head> (above the file's internal <style> so the internal styles can override utilities if absolutely necessary).
Replace hardcoded values:
Replace inline style attributes and internal CSS color/background/border values with var(--token) references.
Replace fixed pixel padding/margins with var(--space-*) where possible.
Apply utility classes to structural containers (sidebars, top bars, property panels, buttons, inputs) to bring in the new visual language (rounded corners, shadows, hover states).
Canvas-specific constraint: If a file uses a <canvas> (levelEditor, enemy_designer, anim_editor):
Only change the background fillStyle of the canvas to match the new ambient base (#050506 or var(--bg-base)).
Leave ALL game/entity rendering logic untouched (platforms, enemies, hitboxes, sprites, grid lines). Changing those colors would alter the functional feedback of the tools—preserve their original color schemes exactly.
Keyboard shortcuts and Undo/Redo must remain completely intact. Do not remove or reassign any keydown listeners.
Test all inputs, sliders, drag interactions, and export/save buttons to ensure they work exactly as before.
...
**Animated Ambient Blobs:** Multiple layered, floating gradient shapes create cinematic lighting. Without these, the design becomes flat and generic.
**Mouse-Tracking Spotlights:** Interactive surfaces respond to cursor position with soft radial glow effects. This creates the "magical" interaction feel.
**Gradient Typography:** Headlines use vertical gradients (white to semi-transparent) and accent gradients with animation for key phrases.
**Multi-Layer Shadows:** Never single shadows. Always combine: border highlight + soft diffuse shadow + optional accent glow.
**Parallax/Scroll Effects:** Hero content fades and scales on scroll. Elements reveal with staggered animations. This adds cinematic depth.
**Precision Micro-Interactions:** All animations are quick (200-300ms), use expo-out easing, and movements are tiny (4-8px). Never bouncy or exaggerated.
---
...
**Pure white text:** Use `#EDEDEF` or similar off-white to reduce harshness.
